...
bool findShopController() {
	...
	if (!patternAddr) {
		write_text_to_log_file("[错误] 未找到 shop_controller 模式 1！");
		return false;
	}
	...
	if (!patternAddr) {
		write_text_to_log_file("[错误] 未找到 shop_controller 模式 2！");
		return false;
	}
	scriptTable = (ScriptTable*)(patternAddr + *(int*)(patternAddr + 3) + 7);

	DWORD startTime = GetTickCount();
	DWORD timeout = 10000; // 以毫秒为单位

	while (!globalTable.IsInitialised()) {
		scriptWait(100); // 继续之前等待全局初始化完成
	...
	if(Item == NULL) { write_text_to_log_file("[错误] ScriptTable 指针为 NULL "); }
...
void enableCarsGlobal() {
	...
	const char* patt1604_0 = "\x2D\x00\x00\x00\x00\x2C\x01\x00\x00\x56\x04\x00\x71\x2E\x00\x01\x62\x00\x00\x00\x00\x04\x00\x71\x2E\x00\x01";
		...
		if (size)
		{
			...
			if (address)
			{
				...
				*globalTable.AddressOf(despawnGlobal) = 1;
				...
				std::stringstream ss; ss << "全局移除: " << despawnGlobal << " 位于地址: " << (void*)address;
